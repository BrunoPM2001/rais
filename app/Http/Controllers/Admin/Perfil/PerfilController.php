...
class PerfilController extends S3Controller {
  public function changePass(Request $request) {
    if ($request->input('new_pass1') == $request->input('new_pass2')) {
      if (Auth::attempt(['username' => $request->attributes->get('token_decoded')->username, 'password' => $request->input('current_password')])) {
        DB::table('Usuario')
          ->where('tabla_id', '=', $request->attributes->get('token_decoded')->id)
          ->update([
            'password' => bcrypt($request->input('new_pass1'))
          ]);

        return ['message' => 'info', 'detail' => 'Contraseña actualizada'];
      } else {
        return ['message' => 'warning', 'detail' => 'La contraseña actual es incorrecta'];
      }
    } else {
      return ['message' => 'warning', 'detail' => 'La contraseña nueva no es la misma en el formulario'];
    }
  }
